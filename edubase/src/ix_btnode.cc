...
// @Parameter IX_BTNode(Left Node)
// @Return RC
// @Function : 왼쪽 노드를 전달받아 현재 노드의 값을 Distribution한다.
RC IX_BTNode::Distribution(IX_BTNode * aNode){
    ...
    // Error Check : 현재 페이지를 받아서 받은 노드의 오른쪽 페이지인지 체크
    PageNum page;
    page = this->GetNodeRID().Page();
    if (page != aNode->GetRight()) {
       return -1;
    }
    
    // 왼쪽 노드에 현재 노드에서 가장 작은값을 옮긴다.
    void *k; //저장될 key값
    this->GetKey(0, k); //현재노드에서의 가장 작은값
    RID rid = this->GetAddr(0); //현재 페이지의 RID주소값
    aNode->InsertNode(k, rid); //성공적으로 insertnode될 경우 0
    
    // 현재 노드에서 옮긴 값을 지운다
    void * keytodel; //지울 key값
    GetKey(0, keytodel); //옮긴 값
    RC rc = DeleteNode(keytodel, -1); //현재 노드에서 값 지우기
    if (rc == -1) return -1; // 실패했을 경우
    
    return OK_RC; //Distribution 성공
}
