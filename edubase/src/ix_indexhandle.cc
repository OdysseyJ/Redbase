...
// @Parameter (void*)pData : 넣을 데이터, (const RID)&rid = 주소, (int)detail = ??
// @Return RC
// @Function : node에 데이터를 Insert해 준다.
RC IX_IndexHandle::InsertEntry(void *pData, const RID &rid, int detail) {
    ...
    int result = node->InsertNode(pData, rid); //성공 = 0, 실패 = -1
    // no room in node - deal with overflow - non-root
    void * failedKey = pData;
    RID failedRid = rid;
    
    // ************************************ 구현 ***********************************
    int isOK_Distribution = false;
    int result_distribute = -1;
    if(result == -1){ //split하기전 distribute가능한지 확인하기 위해 먼저 체크, 노드가 가득 차있음.
        int level = fileHdr.height -1; //leaf level을 의미한다. (insert는 항상 leaf page에 먼저 삽입되니까)
        level--; // 부모레벨로 이동
        
        int rootIsLeaf = false;
        if(level < 0){ //부모가 root
            if(fileHdr.height == 1){ //root이자 leaf page라는 의미
                rootIsLeaf = true; // 무조건 split해야함
            }
        }
        
        if(rootIsLeaf != true){ //만약 루트가 곧 리프페이지면 split, 아니면 아래 로직 진행
            
            IX_BTNode * parent = path[level];
            
            int pos = parent->FindKeyExact((const void*&) pData, rid);
            
            if(pos > 0){ // 왼쪽 형제가 존재할 경우.
                isOK_Distribution = true;
                int posAtSibling = pos -1;
		IX_BTNode* sibling = FetchNode(parent->rids[posAtSibling]); // 내 왼쪽의 이웃노드
        result_distribute = node->Distribution(sibling); //distribution 시도
		node->InsertNode(pData,rid); // distribution 후 실패한 Insert 재 수행.
                
        // 부모의 Key 재설정해주기
		void* key;
		node->GetKey(0, key); // 우측 노드의 가장 작은 키값
		if(result_distribute != -1){ // distribute가 실패하지 않은 경우
		parent->SetKey(posAtSibling, key); // 부모 노드의 키값 재설정
		}
            }
        }
    }
    
    if(result_distribute == -1){ //distribution 시도했는데 만약 왼쪽형제에 자리가 없을경우, (그외의 오류처리)
        isOK_Distribution = false; //split을 해야한다
    }
